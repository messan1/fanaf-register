...
import { CheckCircle, AlertCircle, ChevronRight, ChevronLeft, Building2, Users, CreditCard, Check, Loader2, UserPlus } from "lucide-react";
import { useRegistrationStore } from "@/lib/store/registrationStore";
import { usePlans, useOrganizations, useEventOptions, useRegistration, useValidation, usePricing, useRegistrationData } from "@/lib/hooks/useRegistration";
import { toast } from "sonner";
import { COUNTRIES, getPhoneCodeByCountry } from "@/lib/data/countries";
import { cn } from "@/lib/utils";

const FONCTIONS = [
  "Directeur", "Manager", "Responsable RH", "Commercial", "Chargé de mission", "Consultant", "Chef de projet", "Autre..."
];
const BESOINS = [
  "Networking", "Formation", "Partenariat", "Veille", "Business", "Autre..."
];
...
export default function RegistrationPage() {
  ...
  const [customFonctionsMembers, setCustomFonctionsMembers] = useState<string[]>([]);
  ...
  const { data: eventOptionsData, isLoading: optionsLoading } = useEventOptions();
  const registrationMutation = useRegistration();
  const { validateField, validateForm } = useValidation();
  const eventOptions = (eventOptionsData as any)?.event_options || [];
  const { getBasePrice, getServicesPrice, getTotalPrice } = usePricing(eventOptions);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (mode === 'individual') {
        ...
        requiredFields.forEach(field => {
          const value = groupData.responsable[field as keyof typeof groupData.responsable];
          if (typeof value === 'string') {
            const error = validateField(field, value);
            if (error) {
              validationErrors[field] = error;
        ...
        groupData.members.forEach((member, index) => {
          if (!member.firstName || member.firstName.length < 2) {
            validationErrors[`member_${index}_firstName`] = 'Prénom requis (minimum 2 caractères)';
          }
          if (!member.lastName || member.lastName.length < 2) {
            validationErrors[`member_${index}_lastName`] = 'Nom requis (minimum 2 caractères)';
          }
        ...
        const errorMessages = Object.entries(validationErrors)
          .map(([field, error]) => {
            if (field.startsWith('member_')) {
              ...
              return `Membre ${parseInt(memberIndex) + 2} - ${fieldName}: ${error}`;
            }
            return `${field}: ${error}`;
          })
          .join(', ');
            ...
            {plans.map((plan: any) => (
              <div 
                ...
                className={`cursor-pointer transition-all duration-300 hover:shadow-lg rounded-lg border-2 p-4 ${
                  selectedPlan?.id === plan.id ? 'border-blue-600 bg-blue-50' : 'border-gray-200 bg-white hover:bg-gray-50'
                }`}
                onClick={() => {
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {BESOINS.map(besoin => (
                      <SelectItem key={besoin} value={besoin}>{besoin}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
         ...
         <Label>Organisation *</Label>
             ...
             <div className="bg-gray-50 p-6 rounded-lg">
               <div className="flex items-center justify-between mb-6">
                 <div>
                   <h4 className="text-lg font-semibold text-gray-900 mb-1">Membres du groupe</h4>
                   <p className="text-sm text-gray-600">
                     {groupData.nombre} personne{groupData.nombre > 1 ? 's' : ''} au total
                   </p>
                 </div>
                 <Button
                   type="button"
                   ...
                   return (
                     <div key={idx} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                       ...
                       <div className="grid md:grid-cols-2 gap-4 mb-4">
                         <div>
                           <Label htmlFor={`member-${idx}-firstName`}>Prénom *</Label>
                           <Input
                             id={`member-${idx}-firstName`}
                             placeholder="Prénom"
                             value={member.firstName}
                             onChange={(e) => handleUpdateMember(idx, 'firstName', e.target.value)}
                             className={errors[`member_${idx}_firstName`] ? 'border-red-500' : ''}
                           />
                           <FormError error={errors[`member_${idx}_firstName`]} />
                         </div>
                         
                         <div>
                           <Label htmlFor={`member-${idx}-lastName`}>Nom *</Label>
                           <Input
                             id={`member-${idx}-lastName`}
                             placeholder="Nom"
                             value={member.lastName}
                             onChange={(e) => handleUpdateMember(idx, 'lastName', e.target.value)}
                             className={errors[`member_${idx}_lastName`] ? 'border-red-500' : ''}
                           />
                           <FormError error={errors[`member_${idx}_lastName`]} />
                         </div>
                       </div>
                       
                       <div className="grid md:grid-cols-2 gap-4 mb-4">
                         <div>
                           <Label htmlFor={`member-${idx}-email`}>Email *</Label>
                           <Input
                             id={`member-${idx}-email`}
                             type="email"
                             ...
                             className={errors[`member_${idx}_email`] ? 'border-red-500' : ''}
                           />
                           <FormError error={errors[`member_${idx}_email`]} />
                         </div>
                         
                         <div>
                           <Label htmlFor={`member-${idx}-position`}>Fonction</Label>
                           <Select
                       ...
                       <div>
                         <Label htmlFor={`member-${idx}-besoin`}>Besoin</Label>
                         <Select
                           value={member.besoin}
                           onValueChange={(value) => {
                 ...
                 <div className="flex-1">
                   <div className="flex items-center justify-between mb-2">
                     ...
                     <span className="inline-block px-3 py-1 text-sm font-medium text-gray-700 bg-gray-100 rounded-full">
                       {parseFloat(service.additional_cost).toLocaleString()} FCFA
                     </span>
                   </div>
